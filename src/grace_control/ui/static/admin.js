...
function renderDashboard(){
  ...
  fs.forEach(f=>{
    ...
    const fAppr=f.approval_mode||'auto';
    const apprBadge=fAppr==='auto'?'<span style="color:var(--green);font-weight:600;font-size:9px">AUTO-APPROVE</span>':'<span style="color:var(--yellow);font-weight:600;font-size:9px">MANUAL APPROVAL</span>';
    html+=`<div class="feat-card"><div class="feat-h" onclick="togFeat('${f.id}')"><span class="arrow ${expF?'open':''}">›</span><div class="feat-info"><div class="feat-title">${esc(f.title||f.id)}</div><div class="feat-sub"><span style="color:${fSc};font-weight:700">${fSt}</span>${apprBadge}<span>${f.wave_count||0} waves</span><span>${f.total_packets||0} packets</span>${fAttn?`<span style="color:var(--red)">${fAttn} attention</span>`:''}${fCr?`<span style="color:var(--t3)">created ${fCr}</span>`:''}<span style="color:var(--t3);font-family:var(--mono)">${esc(f.id)}</span></div></div><div class="feat-actions"><button class="feat-act danger" onclick="event.stopPropagation();archiveFeat('${f.id}')" title="Archive">✕</button></div></div>${expF?`<div class="feat-body">${waveHtml}</div>`:''}</div>`;
  });
