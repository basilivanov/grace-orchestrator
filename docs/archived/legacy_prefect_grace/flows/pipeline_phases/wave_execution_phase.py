...
def _select_executor_for_complexity(role: str, complexity: str | None, verification_phase: str | None = None) -> str | None:
    """
    Map role + complexity + verification_phase to executor_id.

    Returns executor_id or None (use default).
    """
    # Use cheap model for lint phase (mechanical fixes)
    if role == "coder" and verification_phase == "lint":
        return "coder-cheap"
    ...
    if role == "coder":
        if complexity == "simple":
            ...
            return "coder-standard"
        elif complexity == "complex":
            return "coder-premium"

    # Architect role (always standard for now)
    if role == "architect":
        return "architect-standard"

    # Other roles use default
    return None
...
def _run_role_packet(
    ...
    # Get packet to check for complexity and verification_phase
    packet = state.packets_by_id.get(packet_id)
    if packet:
        complexity = packet.get("complexity")
        verification_phase = packet.get("verification_phase")
        requested_executor = _select_executor_for_complexity(role, complexity, verification_phase)

        if requested_executor:
            # Update packet with requested executor
            if "execution_hints" not in packet:
                packet["execution_hints"] = {}
            packet["execution_hints"]["requested_executor"] = requested_executor

            # Log the executor request
            import logging
            logger = logging.getLogger(__name__)
            logger.info("EXECUTOR_REQUESTED", extra={
                "packet_id": packet_id,
                "role": role,
