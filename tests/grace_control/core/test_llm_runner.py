...
def _setup_fake_opencode(tmp_path, monkeypatch, exit_code=0, stdout='{"result": "ok"}'):
...
def test_run_llm_resolves_profile_and_returns_stdout(tmp_path, monkeypatch):
    ...
    _setup_fake_opencode(tmp_path, monkeypatch)
    ...
    async def _check():
        result = await run_llm("hello test", role="architect", model="m", cli="opencode", cwd=tmp_path)
        assert "ok" in result
...
def test_run_llm_empty_output_raises(tmp_path, monkeypatch):
    ...
    _setup_fake_opencode(tmp_path, monkeypatch, exit_code=1, stdout="")
    from grace_control.core.llm_runner import run_llm
